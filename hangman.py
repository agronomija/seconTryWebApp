...
class Hangman:
    def __init__(self,secret_word, current_guessed_letter, guessed_letters, number_of_tries, missed_tries ):
    ...
    def check_win(self): #preveri ali je igralec že zmagal
        if len(set(self.secret_word)) == len(set(self.guessed_letters)):
            return True
        return False
    ...
    def sum(self):
        if not self.guessed_letter_valid():
            return 'Vpisati moraš točno eno črko'
...
app = Hangman(secret_word='andraz', current_guessed_letter='a', guessed_letters={'n', 'd', 'r', 'z'}, number_of_tries=0,
              missed_tries=0)
